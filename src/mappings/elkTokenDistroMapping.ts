...
import { ELK_TOKEN_DISTRO } from '../helpers/constants';

export function handleAllocate(event: Allocate): void {
  onAllocate(event, ELK_TOKEN_DISTRO);
}
...
export function handleChangeAddress(event: ChangeAddress): void {
  onChangeAddress(event, ELK_TOKEN_DISTRO);
}

export function handleClaim(event: Claim): void {
  addClaimed(event.params.grantee.toHex(), event.params.amount, ELK_TOKEN_DISTRO);
}
